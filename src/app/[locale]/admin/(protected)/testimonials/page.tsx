...
import { Star, CheckCircle, XCircle, Clock, Search, ChevronLeft, ChevronRight, Eye, Quote, MapPin, Calendar, Loader2 } from "lucide-react";
...
export default function TestimonialsPage() {
  ...
  const filteredTestimonials = testimonials.filter((testimonial) => {
    const matchSearch =
      ...
      testimonial.student.name.toLowerCase().includes(search.toLowerCase()) ||
      (testimonial.location || "").toLowerCase().includes(search.toLowerCase()) ||
      (testimonial.courseName || "").toLowerCase().includes(search.toLowerCase());
    const matchStatus = statusFilter === "all" || testimonial.status === statusFilter;
  ...
  const stats = {
    ...
    avgRating: testimonials.length > 0 ? (testimonials.reduce((sum, item) => sum + item.rating, 0) / testimonials.length).toFixed(1) : "0",
  };
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <Button variant="outline" onClick={() => void fetchTestimonials()}>
            Refresh
          </Button>
        </div>
      </div>
                  ...
                  <Card key={testimonial.id} className="border-0 shadow-sm hover:shadow-lg transition-all">
                    <CardContent className="p-6">
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center gap-3">
                          <div className="w-12 h-12 rounded-full bg-gradient-to-br from-orange-400 to-orange-600 flex items-center justify-center text-white font-bold text-lg">
                            {testimonial.student.name?.split(" ").map((part) => part[0]).join("").slice(0, 2) || "S"}
                          </div>
                          <div>
                      ...
                      <div className="flex flex-wrap gap-2 mb-4">
                        {testimonial.courseName && <Badge variant="outline" className="text-xs">{testimonial.courseName}</Badge>}
                        ...
                        {testimonial.status === "PENDING" && (
                          <>
                            <Button
                              ...
                              onClick={() => void handleModeration(testimonial.id, "APPROVED")}
                              disabled={actionLoading === testimonial.id}
                            >
                              {actionLoading === testimonial.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle className="h-4 w-4 mr-1" />}
                              Approve
                            </Button>
                            <Button
                              variant="outline"
                              ...
                              onClick={() => void handleModeration(testimonial.id, "REJECTED")}
                              disabled={actionLoading === testimonial.id}
                            >
            ...
            <div className="space-y-6 py-4">
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 rounded-full bg-gradient-to-br from-orange-400 to-orange-600 flex items-center justify-center text-white font-bold text-xl">
                  {viewTestimonial.student.name?.split(" ").map((part) => part[0]).join("").slice(0, 2) || "S"}
                </div>
                <div>
                  ...
                  <Badge className={`mt-2 ${statusConfig[viewTestimonial.status].color}`}>{statusConfig[viewTestimonial.status].label}</Badge>
                </div>
              </div>

              <div className="flex items-center gap-1">{renderStars(viewTestimonial.rating)}</div>
                ...
                {viewTestimonial.status === "PENDING" ? (
                  <>
                    <Button
                      ...
                      onClick={() => {
                        void handleModeration(viewTestimonial.id, "REJECTED");
                    ...
                    <Button
                      className="flex-1"
                      onClick={() => {
                        void handleModeration(viewTestimonial.id, "APPROVED");
                        setViewTestimonial(null);
                      }}
                    >
