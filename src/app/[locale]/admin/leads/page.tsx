...
import {
  Search, Mail, Phone, MessageSquare, Clock, CheckCircle, XCircle,
  User, Calendar, Filter, Download, Send, MoreVertical, Eye, Edit,
  ChevronLeft, ChevronRight, Star, TrendingUp, Users, Loader2,
  MapPin, Globe, Clock3, MessageCircle
} from "lucide-react";
...
export default function LeadsPage() {
  ...
  const fetchLeads = async () => {
    ...
    try {
      ...
      // Demo data
      setLeads([
        { id: "1", name: "Sarah Johnson", email: "sarah.j@email.com", phone: "+1234567890", source: "google", course: "200hr YTTC", message: "Interested in March batch", status: "NEW", notes: null, assignedTo: null, followUpAt: null, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
        { id: "2", name: "Michael Chen", email: "mchen@email.com", phone: "+1987654321", source: "instagram", course: "100hr YTTC", message: "Has yoga experience", status: "CONTACTED", notes: "Called - interested in 200hr", assignedTo: "Admin", followUpAt: "2026-05-20", createdAt: new Date(Date.now() - 86400000).toISOString(), updatedAt: new Date().toISOString() },
        { id: "3", name: "Emma Wilson", email: "emma.w@gmail.com", phone: "+441234567890", source: "referral", course: "300hr YTTC", message: "Friend recommended", status: "INTERESTED", notes: "Wants detailed syllabus", assignedTo: "Admin", followUpAt: null, createdAt: new Date(Date.now() - 172800000).toISOString(), updatedAt: new Date().toISOString() },
        { id: "4", name: "James Miller", email: "jmiller@email.com", phone: null, source: "youtube", course: "200hr YTTC", message: null, status: "NOT_INTERESTED", notes: "Looking for cheaper option", assignedTo: null, followUpAt: null, createdAt: new Date(Date.now() - 604800000).toISOString(), updatedAt: new Date(Date.now() - 259200000).toISOString() },
        { id: "5", name: "Anna Mueller", email: "anna.mueller@web.de", phone: "+49123456789", source: "google", course: "200hr YTTC", message: "Professional dancer looking to teach", status: "ENROLLED", notes: "Deposit paid!", assignedTo: "Admin", followUpAt: null, createdAt: new Date(Date.now() - 1209600000).toISOString(), updatedAt: new Date(Date.now() - 432000000).toISOString() },
      ]);
    } finally {
      setLoading(false);
    }
          ...
          <CardContent className="p-4">
            <div className="flex flex-wrap gap-4">
              ...
              >
                ...
                {sources.map(source => (
                  <option key={source} value={source}>{sourceIcons[source] || "📝"} {source}</option>
                ))}
              </select>
                          ...
                          <tr key={lead.id} className="border-b hover:bg-gray-50 transition-colors">
                            <td className="py-4 px-4">
                              <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-orange-400 to-orange-600 flex items-center justify-center text-white font-bold text-sm">
                                  {lead.name?.split(" ").map(n => n[0]).join("").slice(0, 2) || "L"}
                                </div>
                                <div>
                            ...
                            <td className="py-4 px-4">
                              <Badge variant="outline" className="capitalize">
                                <span className="mr-1">{sourceIcons[lead.source] || "📝"}</span>
                                {lead.source}
                              </Badge>
                            </td>
                            <td className="py-4 px-4">
            ...
            <div className="space-y-6 py-4">
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 rounded-full bg-gradient-to-br from-orange-400 to-orange-600 flex items-center justify-center text-white font-bold text-xl">
                  {viewLead.name?.split(" ").map(n => n[0]).join("").slice(0, 2) || "L"}
                </div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="p-4 bg-gray-50 rounded-xl">
                  ...
                  <p className="text-sm font-medium text-gray-900 capitalize">{viewLead.source}</p>
                </div>
                <div className="p-4 bg-gray-50 rounded-xl">
              ...
              <div className="flex gap-3 pt-4 border-t">
                ...
                <Button variant="outline" className="flex-1" onClick={() => { setViewLead(null); handleEdit(viewLead); }}>
                  <Edit className="h-4 w-4 mr-2" />
                  Edit Lead
                </Button>
                <Button className="flex-1" onClick={() => { setViewLead(null); handleEdit(viewLead); }}>
